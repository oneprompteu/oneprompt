# AGENTS.md — Development guidelines for the ThinkingProducts SDK

This document helps AI coding agents (and human contributors) understand and work with the ThinkingProducts Python SDK.

## What is this project?

ThinkingProducts is a Python SDK that provides AI agents for querying PostgreSQL databases with natural language, running Python analysis, and generating interactive charts. It uses Google Gemini as the LLM, LangChain for agent orchestration, and MCP (Model Context Protocol) servers running in Docker containers for tool execution.

**Key user-facing entry points:**
- Python SDK: `import thinkingproducts as tp` → `tp.Client()`
- CLI: `tp init`, `tp start`, `tp stop`, `tp api`
- REST API: FastAPI server at `http://localhost:8000`

## Repository structure

```
thinkingproducts/              # Main Python package
...
├── cli.py                     # Click CLI (tp init/start/stop/status/logs/api)
...
7. `Client` downloads artifacts locally to `tp_data/out/{session}/{run}/`
...
```bash
tp start      # Build and start all Docker containers
tp status     # Check health
...
ruff check .                  # Lint
ruff format .                 # Format
mypy thinkingproducts/        # Type check
```
...
- They share a Docker volume (`tp_exports`) for file access
...
`docker-compose.yml` defines 4 services sharing a `tp_exports` volume. Environment variables are passed from the host `.env`. The `tp start` CLI command wraps `docker compose up --build`.
