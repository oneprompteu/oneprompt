...
Database DSN must be provided in POSTGRES_DSN env var.
...
def _get_dsn_from_context(ctx: Optional[Context] = None) -> Optional[str]:
    ...
    Allows dynamic dataset configuration via x-dataset-dsn header.
    ...
    try:
        ...
        if request_ctx:
            ...
            if request and hasattr(request, "headers"):
                dsn = request.headers.get("x-dataset-dsn")
                if dsn:
                    return dsn
    except Exception:
        pass
...
def _connect(ctx: Optional[Context] = None) -> PgConnection:
    ...
    Priority for DSN:
    1. x-dataset-dsn header from request context (for multi-tenant support)
    ...
    if not dsn:
        raise RuntimeError(
            "No database connection configured. Either provide x-dataset-dsn header "
